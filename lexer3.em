// lexer3.em - 关键字细分词法器：每 Token 输出一行 "<类型码> <行号>"
// Project Basilisk 第三阶段试金石 3：带语义标记的 Token 流（语法分析器最后一道闸门）
// 类型码: 1=通用标识符 2=数字 3=符号
//         10=fn 11=let 12=if 13=else 14=while 15=return 16=extern 17=static
//
// 与规格的关键偏差（必要修正）：str_eq 的 \0 终止假设不成立——
//   缓冲区内标识符后面跟的是源码字符不是 \0，比较必然失配。
//   改用长度感知的 kw_match(start, len, kw)：比 len 字节后验 kw[len]==0。
...
// 长度感知关键字比较：ident 不带 \0，比 len 字节后验 kw[len]==0（.asciz 保证）
// 关键字比 ident 短时，第 len 前某字节必失配（ident 字节恒非 0），天然安全
...
// 按源码出现频率排序：fn/let 最先，extern/static 靠后
        ...
        if in_comment == 1 {
            ...
            if handled == 0 {
                if is_alpha(c) == 1 {
                    ...
                    print_token(classify_ident(buf + st, i - st), line);
                    handled = 1;
                }
            }
            if handled == 0 {
